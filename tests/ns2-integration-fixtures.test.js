...
const mobileArtifact = createArtifact('ns2', canonicalScenario(mobile));
const staticArtifact = createArtifact('ns2', canonicalScenario(fixed));

assert.equal(
  mobileArtifact.text,
  readFixture('integrations/ns2/mobile/mobility-ns2.tcl'),
  'The mobile trace executed by ns-2 must be byte-equivalent to current adapter output.'
);
assert.equal(
  staticArtifact.text,
  readFixture('integrations/ns2/static/mobility-ns2.tcl'),
  'The static trace executed by ns-2 must be byte-equivalent to current adapter output.'
);
...
assert.match(createArtifact('ns2', {
  scenario: 'mobile',
  waypoints: [
...
console.log('ns2-integration-fixtures: mobile/static chains and negative guards passed');
