...
No application server is required. Leaflet, the map rendering library, is bundled locally and does not require network access. Map tiles and place search use external services, so those functions require network access. Optional experimental 3D elevation lookup uses the IoTlab proxy and OpenTopoData/Mapzen; it is disabled by default.
